...
bool TCPSocket::HasHalfClose() const
{
	...
	return (WSAWaitForMultipleEvents(1,&m_hClose,TRUE,0,FALSE)==WSA_WAIT_EVENT_0);
}
//---------------------------------------
int TCPSocket::GetPendingReadData() const
{
	if(!IsConnected()) return -1;
	DWORD uSize;
	int iRet=ioctlsocket(m_xSock, FIONREAD,&uSize);
	return (iRet!=0) ? -1 : uSize;
}
//---------------------------------------
...
//---------------------------------------
bool TCPSocket::WaitRead(int p_iTimeout)
{
	fd_set xFS;
	xFS.fd_count=1;
	xFS.fd_array[0]=m_xSock;
	...
	{
		tv.tv_sec=p_iTimeout/1000;p_iTimeout=p_iTimeout%1000;
		tv.tv_usec=p_iTimeout*1000;
	}
	return (select(0,&xFS,0,0,&tv)==1);
}
//---------------------------------------
...
bool TCPAcceptSocket::Open(IPAddr4 p_xAddr,int p_iMaxPending, int p_iMaxRetriesOnUsed)
{
	do
	{
		...
		if(m_xSock!=INVALID_SOCKET)
		{
			...
			if(bind(m_xSock,(struct sockaddr*)&xAdr,sizeof(xAdr))==0)
			{
			...
			{
				printf("Can't bind accept socket (%d)\r\n", WSAGetLastError());
			}
		}
		Close();
		Sleep(100);
